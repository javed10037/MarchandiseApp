        ...
        if(user.password === ConfirmObj.ConfirmPassword){
            User.findOne({email: user.email},{}, function (err,data) {
                if (err) {
                    ...
                    res.send({message:"User Already Exist With Given Email.",status:400});
                } else {
                    ...
                    crypto.randomBytes(10, function (err, buf) {
                        ...
                        web3.eth.personal.newAccount(user.email).then((data) => {
                            ...
                            if(data){
                                ...
                                User(user).save(function (err, data) {
                                    if (err) {
                                         ...
                                         res.send({status : 400, message : 'Error occured while saving into DB', err : err})
                                    } else {
                                        var verifyurl = 'api/verify-email/' + user.verificationToken;
                                        Mail.registerMail(user,verifyurl, function(msg) {
                                            console.log('Mail sent successfully.')
                                        });
                                        res.status(HttpStatus.OK).send({message: "You have registered successfully. to login please verify your email first.",status:HttpStatus.OK});
                                    }
                                });
                            }
                            else{
                                res.send({data: [],message : 'Error in address generate on ETH Node', status: 400});
                            }
                        });
            ...
            res.json({message : "Password and ConfirmPassword not matched ",status : 400})
        }
    }
}
...
var  UpdateUserProfileById = function(req,res) {
        ...
        User.update(requestParam, { $set : updateParam },function(err,data){
           if(err){
                res.json({message : "Please enter valid ID",status :400})
            }
            else {
            if(data){
               res.json({message : "User profile updated sucessfully",status : 200})
            }
               else {res.json({message : "Please enter the valid inputs",status :400}) }
            }
          })
 }
  ...
  if(req.body.email){
     Contact.create(reqObj,function(err,data){
           if(err){
            ...
            res.json({ message : "Your message send sucessfully to the Cryptual team", status : 200 })
            Mail.ContactUsMail(data, function (msg){
                    console.log("the mail send sucessfully send to contact user")
              });
           }
           else{
             res.json({ message:"Unable to send your info the Cryptual team", status : 400 })
           }
       })
   }else{
...
var subscribeUs = (req,res) => {
   ...
   if(req.body.email){
     Suscribe.findOne({email:req.body.email},{},function(err,data){
       if(err){
         res.json({message : "Please enter the email id first",status : 400})
                ...
                if(sucess){
                  console.log("sucesssssssssssss",sucess);
                   res.json({message : "You successfully subscribe with KRYPTUAL",status : 200})
                    Mail.SubscribeWithUs(sucess, function (msg){
                         console.log("hi this is var smart template")
                    });
...
var saveCrowdsale = (req,res)=>{
    ...
    let userId = req.body.userId;
    console.log('crowdsalecrowdsalecrowdsale',crowdsale);
   Crowdsale.create({crowdsale:crowdsale,userId : userId,token :token})
        ...
        if(result)
           res.json({status:200,result:result,message : 'Saved crowdsale successfully'})
            ...
            res.json({status:400,message:'Error while saving into DB'})
        })
...
var getCrowdsale = (req,res)=>{
    ...
    // console.log('crowdsalecrowdsalecrowdsale',crowdsale);
   Crowdsale.findOne({userId : userId},{})
        .then((result)=>{
        if(result)
           res.json({statusCode:200,result:result,message : 'get'})
        else
            res.json({statusCode:400,message:'invalid coupon'})
        })
    .catch((unsuccess)=>{res.json({message:'something went wrong',statusCode:400,unsuccess:unsuccess})})
...
var deleteCrowdsale = (req,res)=>{
    var userId = req.body.userId;
    console.log('deleteCrowdsale',userId);
   Crowdsale.remove({userId : userId},{},function(err, result) {
            if (err) {
                console.log('inside err',userId);
   ...
   // res.json({statusCode:200,message : 'delete'})
    //     .then((result)=>{
    //     if(result)
    //        res.json({statusCode:200,result:result,message : 'delete'})
    //     else
    //         res.json({statusCode:400,message:'invalid coupon'})
    //     })
    // .catch((unsuccess)=>{res.json({message:'something went wrong',statusCode:400,unsuccess:unsuccess})})
...
var getTopIcoByAdmin = (req,res)=>{
    ...
    .then((success)=>{
        ...
        // var data = {};
        //     console.log('Constant.ethplorer',Constant.ethplorer);
        //     opt.url = Constant.ethplorer+'getTokenInfo'+success.tokenAddress
